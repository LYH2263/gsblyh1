...
InsightBoard 是一个面向课程项目的数据可视化全栈应用，支持数据录入/导入、统计分析、图表展示、明细下钻与 CSV 导出。

## 原始需求

> 应用Vue.js框架技术，设计并开发一个具有创新性和实际应用价值的前端应用，作为大学期末作品。该应用需满足以下要求：
> 选题应具备一定的新颖性，避免简单的CRUD功能重复实现，可结合当前技术趋势如AI交互、数据可视化、响应式设计或特定领域解决方案
> 应用需包含至少5个以上完整的功能模块，实现前后端数据交互，可使用Vue 3 + Composition API + Vite构建
> 技术栈应包含Vue Router实现路由管理，Pinia或Vuex进行状态管理，Axios处理网络请求，以及至少一种UI组件库
> 需实现响应式设计，确保在不同设备上均有良好的显示效果和用户体验
> 代码需结构清晰、注释完整，符合前端开发规范，包含README文档说明项目背景、功能介绍、技术栈、安装部署步骤
> 需提供至少3个测试用例，验证核心功能的正确性和稳定性
> 最终提交物应包括完整的源代码、可运行的项目构建包、详细的开发文档和演示视频或截图
