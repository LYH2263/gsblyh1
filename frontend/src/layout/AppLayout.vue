...
const navItems = computed(() => {
  ...
  const explorePath = currentDatasetId.value
    ...
    : '/app/datasets';
  const qualityPath = currentDatasetId.value
    ? `/app/datasets/${currentDatasetId.value}/quality`
    : '/app/quality';
  ...
  return [
    ...
    { label: '质量巡检', to: qualityPath, match: 'quality' as const },
...
const isNavActive = (item: { to: string; match: string }) => {
  ...
  switch (item.match) {
    ...
    case 'quality':
      return path.endsWith('/quality') || path.includes('/quality');
    case 'settings':
